...
class Slider_Controller extends CI_Controller
{
	...
	{
		$id = $this->uri->segment(3);
		$record=$this->Slider_Model->fetch_with_condition('nqash_cms.tblsliders', 'sliderid', $id);
		if (file_exists(FCPATH."assets/upload_sliders/".$record[0]->Image)) {
			unlink(FCPATH."assets/upload_sliders/".$record[0]->Image);
		 }
	...
	public function edit_data()
	{
		...
		if (strlen($_FILES['file']['name']) != 0) {
			if (file_exists(FCPATH."assets/upload_sliders/".$_POST['Image_text'])) {
				unlink(FCPATH."assets/upload_sliders/".$_POST['Image_text']);
			 }
